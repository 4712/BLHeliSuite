...
#endif
#ifdef _PB0
.equ	RCP_PORT		= PORTB		; Receive port and pin
.equ	rcp_in			= 0
.equ	RCP_PIN 		= PINB
.equ	RCP_DDR 		= DDRB  
#endif
...
#endif
#ifdef _PD2
.equ	RCP_PORT		= PORTD		; Receive port and pin
.equ	rcp_in			= 2
.equ	RCP_PIN 		= PIND
.equ	RCP_DDR 		= DDRD  
#endif
...
#if defined(_M168DEF_INC_)||defined(_M168PADEF_INC_)
.set	SPMEN	= SELFPRGEN		;SPMEN to SELFPRGEN
.set    SPMCR	= SPMCSR		;SPMCR to SPMCSR
.set	EEMWE	=EEMPE			;EEMWE to EEMPE
.set	EEWE	=EEPE			;EEWE to EEPE

.MACRO StartTimer
	ldi	r20, (1<<CS01)		; timer2: clk/8 ... 256 ticks @ 16MHz = 128us; @ 8MHz = 256us
	out	TCCR0B, r20
.ENDMACRO

.MACRO ldi_r20_TOV_OCF
	ldi	r20, (1<<TOV0)+(1<<OCF0B)
.ENDMACRO
...
.MACRO Set7_8thbitTime
	out	TCNT0, r8
.ENDMACRO

.MACRO SetHalf_bit_high_time
	out	TCNT0, r3
.ENDMACRO

.MACRO SetHalf_bit_low_time
	out	TCNT0, r2
.ENDMACRO

.MACRO SetOCR_quarter
	out	OCR0B, r4
.ENDMACRO
...
.org BOOT_START
boot_reset:	
		ldi	ZL, high(RAMEND)	; Set up stack
		ldi	ZH, low(RAMEND)
		out	SPH, ZH
		out	SPL, ZL
...
boot_rx_time:	
		...
		;out	TCNT2, r16
		ClearTimerFlags
		;ClearTimerFlags ;out	TIFR, r20
boot_rx_time1:	
		cpi	XL, low(TX_BUFFER)
		breq	boot_rx_no_tx
		in_r14_TIFR
		;in	r14, TIFR
		sbrc	r14, TOV2
...
boot_exit:	
		...
		;out	TCNT2, r16
		ClearTimerFlags
		;ClearTimerFlags ;out	TIFR, r20		; Start measuring high time
boot_rx_time3:	
		in_r14_TIFR;in	r14, TIFR
		sbrc	r14, TOV2
		...
		rjmp	boot_rx_time3
		in_r3_TCNT 
		;in	r3, TCNT0		; Save learned high time
		ResetTimer
		;out	TCNT2, r16
		ClearTimerFlags
		;ClearTimerFlags ;out	TIFR, r20		; Start measuring low time
boot_rx_time4:	
		in_r14_TIFR;in	r14, TIFR
		sbrc	r14, TOV2
		rjmp	FLASHEND + 1		; Low too long, exit boot loader
...
boot_rx_bit:	
		ror	r22			; Roll rx bit in carry into r22
...
boot_tx_bits:	
		lsr	r22			; Put next bit in carry flag
		sbi	RCP_PORT, rcp_in	; Drive high
		SetHalf_bit_high_time 
		; out	TCNT2, r3
		ClearTimerFlags ;out	TIFR, r20
...
boot_rx_cont:	
		ldi	r24, 0
boot_rx_skip:	
		ldi	r22, 0b10000000		; Restart with sentinel bit preloaded
		rjmp	boot_rx
...
stk_rx_seq:	
		...
		rjmp	boot_rx_cont
stk_rx_size_h:	
		mov	r7, r22			; Store message length high in r7
		eor	r5, r22
		adiw	ZL, stk_rx_size_l - stk_rx_size_h
		rjmp	boot_rx_cont
stk_rx_size_l:	
		mov	r6, r22			; Store message length low in r6
		eor	r5, r22
...
scmd_multi1:	
		st	X+, r16			; Fill return buffer with zeroes
		dec	r24
		brne	scmd_multi1
	...
	; Mirror address in result
		...
		brne	scmd_multi3
scmd_multi2:	
		std	Z+10, r24		; Signature byte
scmd_multi3:	
		st	X+, r16			; STATUS_CMD_OK
...
scmd_read_eeprom:
		...
		ldi	r17, (1<<EERE)
scmd_read_ee1:	
		rcall	boot_eeprom_rw		; Uses and increments byte address
		in	r22, EEDR
		st	X+, r22
		sbiw	r24, 1
		brne	scmd_read_ee1
...
scmd_spm_wait:	
		in	r23, SPMCR		; Wait for previous SPM to finish
		sbrc	r23, SPMEN
		rjmp	scmd_spm_wait
scmd_ee_wait:	
		sbic	EECR, EEWE		; Wait for EEPROM write to finish
		rjmp	scmd_ee_wait
		ret
scmd_spm:	
		rcall	scmd_spm_wait
